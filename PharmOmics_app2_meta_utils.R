# functions for PharmOmics App 2 (Drug network repositioning)
...
preparenetpermutation <- function(genes,net,DOBtable){
  ...
  if(sum(class(finalsample) %in% "character")>0){
...
calculatenetworkscore <- function(drugDEG, diseasegene, permutedrugDEG,permutediseaseDEG,alldistancetable){
  finalresult <- NULL
  allscores <- NULL
  for(k in 1:1000){
    if(length(drugDEG) == 1){
...
prepareplot <- function(diseasegenes,druggene,net,filelocation,drugname){
...
getResult <- function(diseasegenes, full_genes, net, DOBtable, finalKDAframe, 
                      alldistancetable, drugNetworksDir, species){
  randomgenes_disease <- preparenetpermutation(diseasegenes,net,DOBtable)
  ...
  for(j in 1:nrow(finalKDAframe)){
    ...
    changegenes <- unique(c(unlist(strsplit(finalKDAframe$Genes_up[j],",")),unlist(strsplit(finalKDAframe$Genes_down[j],","))))
    
    if(length(changegenes)==0){ # no genes at all - shouldn't be the case for meta signatures
      Jaccard_resd[j] <- NA
      resd[j] <- NA
    ...
    else if(sum(changegenes %in% V(net)$name)==0){ # no drug genes in the network
      resd[j] <- NA
  ...
  tableresult <- data.frame(Z_score = resd,
                            Rank = z_scorerank,
                            Pvalue = z_scorep_pvalue, 
                            All_Drug_Gene_Input_Gene_Overlap=direct_overlap, 
                            Drug_gene_input_gene_overlap_in_network=drug_input_gene_network_overlap,
                            Drug_genes_directly_connected_to_input_gene=one_neighbor_overlap,
                            Input_genes_directly_connected_to_drug_gene=input_neighbor_to_drug_gene)
  tableresult <- cbind(finalKDAframe, tableresult)
  tableresult <- tableresult[order(tableresult$Pvalue),]
  
  if(species=="Mouse"){
    tableresult$Drug_network_name <- paste(tableresult$Drug, tableresult$Tissue, tableresult$Species, sep = "_")
  }
  else{
    tableresult$Drug_network_name <- paste(tableresult$Drug, tableresult$Tissue, sep = "_")
  }
  
  # take top 50 to create networks
  for(i in 1:50){
    druggene <- unique(c(unlist(strsplit(tableresult$Genes_up[i],",")),unlist(strsplit(tableresult$Genes_down[i],","))))
    if(length(druggene)>0){
      prepareplot(diseasegenes = diseasegenes, druggene = druggene,
                  net = finalnet, filelocation = drugNetworksDir, drugname=tableresult$Drug_network_name[i])
    }
  }
